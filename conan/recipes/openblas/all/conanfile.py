...
class OpenblasConan(ConanFile):
    ...
    def configure(self):
        ...
        if cross_building(self, skip_x64_x86=True) and not self.options.target:
            ...
            if target:
                self.output.warning(
                    f'Setting OpenBLAS TARGET={target} based on settings.arch. This may result in suboptimal performance. Set the "{self.name}/*:target=XXX" option to silence this warning.'
                )
    ...
    def validate(self):
        if Version(self.version) < "0.3.24" and self.settings.arch == "armv8":
            ...
            raise ConanInvalidConfiguration(
                "armv8 builds are not currently supported for versions lower than 0.3.24. Contributions to support this are welcome."
            )

        if self.options.build_relapack:
            if not self.options.build_lapack:
                raise ConanInvalidConfiguration(
                    f'"{self.name}/*:build_relapack=True" option requires "{self.name}/*:build_lapack=True"'
                )
            if self.settings.compiler not in ["gcc", "clang"]:
                # ld: unknown option: --allow-multiple-definition on apple-clang
                raise ConanInvalidConfiguration(
                    f'"{self.name}/*:build_relapack=True" option is only supported for GCC and Clang'
                )
    ...
    def validate_build(self):
        if Version(self.version) < "0.3.22" and cross_building(self, skip_x64_x86=True):
            ...
            raise ConanInvalidConfiguration(
                f"Cross-building is not supported for {self.name}/0.3.21 and earlier."
            )
        ...
        if cross_building(self, skip_x64_x86=True) and not self.options.target:
            raise ConanInvalidConfiguration(
                f'Could not determine OpenBLAS TARGET. Please set the "{self.name}/*:target=XXX" option.'
            )
    ...
    def _patch_sources(self):
        if Version(self.version) <= "0.3.15":
            replace_in_file(
                self,
                os.path.join(self.source_folder, "cmake", "utils.cmake"),
    ...
    def package_info(self):
        ...
        self.cpp_info.components["openblas_component"].set_property(
            "cmake_target_name", f"OpenBLAS::{cmake_component_name}"
        )
        self.cpp_info.components["openblas_component"].set_property(
